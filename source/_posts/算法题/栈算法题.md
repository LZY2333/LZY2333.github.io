...
class MinStack {
    ...
    push(val: number): void {
        this.Stack.push(val);
        this.MinStack.push(Math.min(val, this.getMin() ?? Number.NEGATIVE_INFINITY ));
    }
...
[leetcode](https://leetcode.cn/problems/evaluate-reverse-polish-notation/description/)
...
## 11.柱状图中最大的矩形（ LeetCode 84 ）

[leetcode](https://leetcode.cn/problems/largest-rectangle-in-histogram/)

对每一个柱子找到左边界和右边界，也就是比自己小的数字。

利用单调递增栈，发现比自己小的数字，就是右边界。
...
var largestRectangleArea = function (heights) {
    ...
    for (let i = 0; i < h.length; i++) {
        while (stack.length && h[i] < h[stack[stack.length - 1]]) {
            const cur = stack.pop()
            res = Math.max(res, h[cur] * (i - stack[stack.length - 1] - 1))
        }
        stack.push(i)
    }
    return res
