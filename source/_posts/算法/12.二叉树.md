...
```js
// 层序遍历，result每个i是一层的数据
var levelOrder = function(root) {
    if(!root) return []
    let queue = [root]
    let result = []
    while(queue.length !== 0) {
        const l = queue.length // 队列前l个就是当前层的数据
        result.push([])
        for(let i = 0; i < l; i++) {
            const p = queue.shift()
...
[carlsun-2大佬的题解](https://leetcode-cn.com/problems/binary-tree-postorder-traversal/solution/bang-ni-dui-er-cha-shu-bu-zai-mi-mang-che-di-chi-t/)
